...
def getPage(index):
    ...
    dom = pq(page.text)
    tableNode = dom(".profile")(".row").eq(2)("table")
    return tableNode
    pass
...
def getPageInfo(url):
    ...
    page = requests.get(hurl)
    dom = pq(page.text)
    profile = dom(".profile")(".row").eq(1)
    regex = re.compile(r'Showing the last (\d+) verified contract source code')
    ...
    lastHref = profile.children(".col-sm-6").eq(1)("a").eq(3).attr("href")
    print('lastHref', str(lastHref))
    # lastregex = re.compile(r'contractsVerified/(\d+)')
    # m = lastregex.match(lastHref)
    pageSize = str(lastHref).split('/')[-1]
    # if m:
    #     pageSize = m.group(1)
    #     print(pageSize)
